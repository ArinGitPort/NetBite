...
export const chapterFour = createAdvancedChapter({
  ...
  lessons: [
    {
      ...
      example: { label: 'TWO IDENTITIES', setup: 'PC1 sends an IPv4 datagram through a router to a remote PC.', presentation: 'guided', visual: { illustration: 'ipv4-address', stageIds: ['ip-endpoints', 'first-link', 'router', 'second-link'] }, steps: [
        { id: 'ip-endpoints', label: 'SET THE IP ENDPOINTS', explanation: 'The IPv4 header identifies PC1 as the source and the remote PC as the destination. Routers later inspect the destination IPv4 address.', value: 'SOURCE IP A / DESTINATION IP B' },
        { id: 'first-link', label: 'BUILD THE FIRST FRAME', explanation: 'PC1 chooses its local next hop and resolves that interface\'s MAC address. The Ethernet frame carries PC1\'s source MAC and the next-hop destination MAC.', value: 'LOCAL-LINK MAC ADDRESSES' },
        { id: 'router', label: 'CROSS THE ROUTER', explanation: 'The router removes the incoming Ethernet framing, inspects the destination IPv4 address, and selects another outgoing link.', value: 'IP DESTINATION STILL PC2' },
  ...
  questions: [
    ...
    { lessonId: 'private-valid-hosts', prompt: 'PC2 receives the same IPv4 address as PC1. What should change?', answers: ['Assign PC2 a unique usable address', 'Keep both because their MACs differ', 'Rename PC2 only'], correctAnswerIndex: 0, explanation: 'Host IPv4 addresses must be unique within the subnet.' },
  ],
  cards: [
