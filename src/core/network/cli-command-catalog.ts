...
export const cliCommandCatalog: CanonicalCliCommand[] = [
  ...
  { id: 'cli-show-running-config', command: 'show running-config', description: 'Inspect the current device configuration.' },
  { id: 'cli-show-ip-interface-brief', command: 'show ip interface brief', description: 'Summarize interface addressing and state.' },
  ...
  { id: 'cli-ping', command: 'ping <destination>', description: 'Run an IPv4 reachability test.' },
  ...
  { id: 'cli-ip-route', command: 'ip route <network> <mask> <next-hop>', description: 'Create a static route supported by this practice.' },
