    ...
    <View style={styles.panel}>
      ...
      {device.type !== 'switch' ? (
        <View style={styles.formBlock}>
          ...
          <View style={styles.row}><Field label="IPV4 ADDRESS" value={ip} placeholder="192.168.1.10" onChangeText={setIp} /><Field label="PREFIX" value={prefix} placeholder="24" keyboardType="numeric" onChangeText={setPrefix} /></View>
          {device.type === 'pc' ? <Field label="DEFAULT GATEWAY" value={gateway} placeholder="192.168.1.1" onChangeText={setGateway} /> : null}
        ...
        <View style={styles.formBlock}>
          ...
          {selected.switchportMode === 'trunk' ? <><Field label="ALLOWED VLANS" value={allowed} placeholder="10,20" onChangeText={setAllowed} /><AppButton label="Save allowed VLANs" onPress={saveAllowed} /></> : <><Field label="ACCESS VLAN" value={String(selected.accessVlan ?? 1)} placeholder="10" keyboardType="numeric" onChangeText={(value) => onConfigure({ interfaceId: selected.id, interface: { accessVlan: Number(value) } })} /></>}
          <Field label="VLAN DATABASE" value={vlans} placeholder="10,20" onChangeText={setVlans} /><AppButton label="Save VLAN database" variant="secondary" onPress={saveVlans} />
        </View>
      )}
      {device.type === 'router' ? <View style={styles.formBlock}><Text variant="label" style={styles.blockTitle}>STATIC ROUTES</Text>{device.routes.map((route, index) => <View key={`${route.prefix}-${index}`} style={styles.record}><Text variant="technical" style={styles.recordText}>{route.prefix}/{route.prefixLength} VIA {route.nextHop}</Text><Pressable accessibilityRole="button" onPress={() => onConfigure({ routes: device.routes.filter((candidate) => candidate !== route) })}><Text variant="technical" style={styles.removeText}>REMOVE</Text></Pressable></View>)}<View style={styles.row}><Field label="NETWORK" value={routeNetwork} placeholder="10.0.2.0" onChangeText={setRouteNetwork} /><Field label="PREFIX" value={routePrefix} placeholder="24" keyboardType="numeric" onChangeText={setRoutePrefix} /></View><Field label="NEXT HOP" value={routeNextHop} placeholder="10.0.1.2" onChangeText={setRouteNextHop} /><AppButton label="Add static route" onPress={addRoute} /></View> : null}
...
const styles = StyleSheet.create({
  ...
  input: { minHeight: 44, borderWidth: 1, borderColor: Palette.border, backgroundColor: Palette.background, color: Palette.text, paddingHorizontal: Space.md, paddingVertical: Space.sm, fontFamily: Fonts.mono, ...Typography.bodySmall },
