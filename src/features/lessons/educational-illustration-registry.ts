...
export const educationalIllustrations: Record<string, EducationalIllustrationSpec> = {
  ...
  'default-gateway': spec({ id: 'default-gateway', family: 'sequence', title: 'REMOTE NEXT HOP', accessibilityLabel: 'PC1 sends a remote destination toward gateway 192.168.10.1, which is reachable on PC1’s local subnet.', sourceIds: ['RFC-1122'], nodes: [
    { label: 'PC1', detail: '192.168.10.10/24', token: 'pc', tone: 'blue' }, { label: 'DEFAULT GATEWAY', detail: '192.168.10.1 / LOCAL', token: 'router', tone: 'orange' }, { label: 'REMOTE LAN', detail: '192.168.20.0/24', token: 'pc', tone: 'sage' },
  ...
  'arp-request': spec({ id: 'arp-request', family: 'packet-fields', title: 'ARP REQUEST INSIDE ETHERNET', accessibilityLabel: 'PC1 sends an ARP Request in an Ethernet broadcast frame. The Ethernet destination is FF:FF:FF:FF:FF:FF, EtherType is 0x0806, and the ARP message asks for the MAC owning 192.168.10.20.', sourceIds: ['RFC-826', 'IANA-ETHERTYPES'], protocolGroups: [
    { id: 'ethernet-envelope', label: 'OUTER ETHERNET FRAME', detail: 'DELIVERS THE QUESTION LOCALLY', fields: [
      ...
      { id: 'arp-eth-source', label: 'SOURCE MAC', value: '02:00:00:00:00:0A', detail: 'PC1 sent this frame.', tone: 'blue' },
    ...
    { id: 'arp-message', label: 'ARP REQUEST PAYLOAD', detail: 'ASSERTION ABOUT A / QUESTION ABOUT B', fields: [
      ...
      { id: 'arp-sender', label: 'SENDER IPv4 + MAC', value: '192.168.10.10 / 02:00:00:00:00:0A', detail: 'PC1 identifies itself so peers can learn it.', tone: 'blue' },
    ...
    { id: 'build', title: 'BUILD THE ARP QUESTION', accessibilityLabel: 'PC1 supplies its own IPv4 and MAC plus target IPv4 192.168.10.20.', activeFieldIds: ['arp-operation', 'arp-sender', 'arp-target-ip', 'arp-target-mac'] },
    { id: 'broadcast', title: 'BROADCAST THE ETHERNET FRAME', accessibilityLabel: 'The unknown target MAC causes PC1 to use Ethernet destination FF:FF:FF:FF:FF:FF with EtherType 0x0806.', activeFieldIds: ['arp-eth-destination', 'arp-eth-source', 'arp-ethertype'] },
  ...
  'arp-reply': spec({ id: 'arp-reply', family: 'packet-fields', title: 'ARP REPLY AND CACHE UPDATE', accessibilityLabel: 'PC2 normally sends a unicast ARP Reply to PC1, identifying 192.168.10.20 as MAC 02:00:00:00:00:0B, and PC1 stores the mapping.', sourceIds: ['RFC-826'], protocolGroups: [
    { id: 'reply-frame', label: 'OUTER ETHERNET FRAME', detail: 'DIRECT RESPONSE TO THE REQUESTER', fields: [
      { id: 'reply-eth-destination', label: 'DESTINATION MAC', value: '02:00:00:00:00:0A', detail: 'Normally unicast to PC1.', tone: 'blue' },
      { id: 'reply-eth-source', label: 'SOURCE MAC', value: '02:00:00:00:00:0B', detail: 'PC2 owns the requested address.', tone: 'sage' },
    ...
    { id: 'reply-message', label: 'ARP REPLY PAYLOAD', fields: [
      ...
      { id: 'reply-mapping', label: 'SENDER MAPPING', value: '192.168.10.20 / 02:00:00:00:00:0B', detail: 'The fact PC1 needed to build its data frame.', tone: 'sage' },
      { id: 'reply-target', label: 'REQUESTER', value: '192.168.10.10 / 02:00:00:00:00:0A', detail: 'The response is directed back to PC1.', tone: 'blue' },
    ] },
  ], stages: [
    { id: 'learn-requester', title: 'PC2 CAN LEARN PC1 FROM THE REQUEST', accessibilityLabel: 'The original request carried PC1 sender IPv4 and MAC, allowing PC2 to learn that mapping.', activeFieldIds: ['reply-target'] },
    { id: 'build-reply', title: 'PC2 BUILDS THE REPLY', accessibilityLabel: 'PC2 sets operation Reply and supplies its own IPv4 to MAC mapping.', activeFieldIds: ['reply-operation', 'reply-mapping'] },
    { id: 'unicast', title: 'REPLY NORMALLY RETURNS BY UNICAST', accessibilityLabel: 'PC2 normally sends the Ethernet frame directly to PC1 MAC.', activeFieldIds: ['reply-eth-destination', 'reply-eth-source', 'reply-ethertype'] },
    { id: 'cache', title: 'PC1 STORES THE MAPPING', accessibilityLabel: 'PC1 stores 192.168.10.20 mapped to 02:00:00:00:00:0B for later frame delivery.', activeFieldIds: ['reply-mapping'] },
  ...
  'access-port': spec({ id: 'access-port', family: 'sequence', title: 'ACCESS PORT MEMBERSHIP', accessibilityLabel: 'A PC attaches to one switch access port assigned to VLAN 10; the endpoint traffic is associated with that VLAN.', sourceIds: ['IEEE-802.1Q', 'CISCO-VLAN'], nodes: [
    { label: 'PC1', detail: 'ENDPOINT', token: 'pc', tone: 'blue' }, { label: 'ACCESS PORT', detail: 'ONE ASSIGNED VLAN', token: 'switch', tone: 'sage' }, { label: 'VLAN 10', detail: 'PORT MEMBERSHIP', token: 'vlan-tagged-frame', tone: 'blue' },
  ] }),
  ...
  'same-vlan': spec({ id: 'same-vlan', family: 'topology', title: 'SAME-VLAN LAYER 2 PATH', accessibilityLabel: 'Two PCs in VLAN 10 can use a Layer 2 switching path when access membership and every link carrying VLAN 10 are valid.', sourceIds: ['IEEE-802.1Q', 'CISCO-VLAN'], nodes: [
    { label: 'PC1 / VLAN 10', token: 'pc', tone: 'blue' }, { label: 'SWITCHING PATH', detail: 'VLAN 10', token: 'switch', tone: 'sage' }, { label: 'PC2 / VLAN 10', token: 'pc', tone: 'blue' },
  ] }),
  ...
  'inter-vlan-boundary': spec({ id: 'inter-vlan-boundary', family: 'sequence', title: 'CROSS A VLAN BOUNDARY', accessibilityLabel: 'PC1 in VLAN 10 reaches PC2 in VLAN 20 only through a Layer 3 router; the two Layer 2 broadcast domains remain separate.', sourceIds: ['IEEE-802.1Q', 'RFC-1812', 'CISCO-INTER-VLAN'], nodes: [
    { label: 'PC1 / VLAN 10', detail: '192.168.10.10/24', token: 'pc', tone: 'blue' }, { label: 'R1 / LAYER 3', detail: 'ROUTES BETWEEN NETWORKS', token: 'router', tone: 'orange' }, { label: 'PC2 / VLAN 20', detail: '192.168.20.20/24', token: 'pc', tone: 'gold' },
  ], footer: 'ROUTING CROSSES THE BOUNDARY / IT DOES NOT MERGE THE VLANS' }),
  'vlan-gateway': spec({ id: 'vlan-gateway', family: 'comparison', title: 'ONE LOCAL GATEWAY PER VLAN', accessibilityLabel: 'VLAN 10 host PC1 uses gateway 192.168.10.1 while VLAN 20 host PC2 uses gateway 192.168.20.1; each gateway is in its host local subnet.', sourceIds: ['RFC-1122', 'RFC-1812', 'CISCO-INTER-VLAN'], nodes: [
  ...
  'router-on-stick': spec({ id: 'router-on-stick', family: 'topology', title: 'ROUTER-ON-A-STICK', accessibilityLabel: 'PC1 and PC2 use separate access VLANs on one switch. Switch port F0/24 is an 802.1Q trunk to one physical router interface G0/0.', sourceIds: ['IEEE-802.1Q', 'CISCO-INTER-VLAN'], nodes: [
    { label: 'VLAN 10 ACCESS', detail: 'PC1 / F0/1', token: 'pc', tone: 'blue' }, { label: 'SW1', detail: 'F0/24 TRUNK / ALLOW 10,20', token: 'switch', tone: 'sage' }, { label: 'R1 G0/0', detail: 'ONE PHYSICAL LINK', token: 'router', tone: 'orange' }, { label: 'VLAN 20 ACCESS', detail: 'PC2 / F0/2', token: 'pc', tone: 'gold' },
  ] }),
  ...
  'inter-vlan-forwarding': spec({ id: 'inter-vlan-forwarding', family: 'sequence', title: 'FRAME REPLACEMENT ACROSS VLANs', accessibilityLabel: 'PC1 selects its VLAN 10 gateway, the tagged frame reaches R1, R1 routes to connected VLAN 20, creates new Ethernet framing, and PC2 returns through its VLAN 20 gateway.', sourceIds: ['RFC-826', 'RFC-1812', 'IEEE-802.1Q'], nodes: [
    { label: 'PC1', detail: 'DESTINATION IP 192.168.20.20', token: 'pc', tone: 'blue' }, { label: 'VLAN 10 TRUNK FRAME', detail: 'NEXT HOP 192.168.10.1', token: 'vlan-tagged-frame', tone: 'blue' }, { label: 'R1 ROUTES', detail: 'MATCH 192.168.20.0/24', token: 'router', tone: 'orange' }, { label: 'VLAN 20 DELIVERY', detail: 'NEW ETHERNET FRAME', token: 'vlan-tagged-frame', tone: 'gold' }, { label: 'PC2 REPLIES', detail: 'RETURN VIA 192.168.20.1', token: 'pc', tone: 'gold' },
  ], stages: guidedStages({ gateway: 'CHOOSE THE VLAN 10 GATEWAY', 'tag-in': 'CARRY VLAN 10 TO R1', route: 'SELECT THE VLAN 20 ROUTE', 'tag-out': 'BUILD VLAN 20 DELIVERY', reply: 'VERIFY THE RETURN PATH' }) }),
  ...
  'dhcp-discover': spec({ id: 'dhcp-discover', family: 'topology', title: 'DISCOVER STAYS IN THE LOCAL VLAN', accessibilityLabel: 'PC1 broadcasts DHCP Discover. SW1 floods it inside VLAN 10. R1 does not forward the original broadcast without relay service.', sourceIds: ['RFC-2131', 'IEEE-802.1Q'], stages: basicLessonStages, nodes: [
    { label: 'PC1', detail: 'DHCPDISCOVER / BROADCAST', token: 'pc', tone: 'blue' }, { label: 'SW1', detail: 'FLOOD INSIDE VLAN 10', token: 'switch', tone: 'orange' }, { label: 'R1', detail: 'BROADCAST BOUNDARY', token: 'router', tone: 'red' },
  ...
  'dhcp-request-ack': spec({ id: 'dhcp-request-ack', family: 'sequence', title: 'THE CLIENT SELECTS / THE SERVER CONFIRMS', accessibilityLabel: 'After receiving an offer, PC1 sends DHCP Request for 192.168.10.101. DHCP1 replies with DHCP Acknowledge, allowing the client to install the settings.', sourceIds: ['RFC-2131', 'RFC-2132'], stages: basicLessonStages, nodes: [
    { label: 'OFFER RECEIVED', detail: '192.168.10.101 PROPOSED', token: 'pc', tone: 'orange' }, { label: 'DHCPREQUEST', detail: 'CLIENT SELECTS THE OFFER', token: 'transport-channel', tone: 'gold' }, { label: 'DHCPACK', detail: 'SERVER COMMITS THE LEASE', token: 'server-terminal', tone: 'sage' }, { label: 'PC1', detail: 'INSTALLS THE SETTINGS', token: 'pc', tone: 'blue' },
  ] }),
  'dhcp-pool': spec({ id: 'dhcp-pool', family: 'table', title: 'POOL AVAILABILITY', accessibilityLabel: 'A two-address DHCP pool has both addresses leased, so PC3 cannot receive an offer until an address is released or the pool changes.', sourceIds: ['RFC-2131', 'RFC-2132'], stages: basicLessonStages, headers: ['ADDRESS', 'BINDING', 'AVAILABLE'], rows: [
    ['192.168.10.101', 'PC1', 'NO'], ['192.168.10.102', 'PC2', 'NO'], ['NEXT REQUEST', 'PC3', 'POOL EXHAUSTED'],
  ] }),
  'dhcp-relay': spec({ id: 'dhcp-relay', family: 'topology', title: 'DHCP RELAY PATH', accessibilityLabel: 'PC2 broadcasts on the 192.168.20.0 slash 24 client network. SW1 delivers it to R1, which relays it to DHCP1 on 192.168.10.0 slash 24.', sourceIds: ['RFC-2131', 'RFC-2132'], stages: basicLessonStages, nodes: [
    { label: 'PC2', detail: '192.168.20.0/24 CLIENT', token: 'pc', tone: 'blue' }, { label: 'SW1', detail: 'CLIENT VLAN', token: 'switch', tone: 'orange' }, { label: 'R1 RELAY', detail: '192.168.20.1', token: 'router', tone: 'gold' }, { label: 'DHCP1', detail: 'SELECT 192.168.20.0/24 POOL', token: 'server-terminal', tone: 'sage' },
  ] }),
