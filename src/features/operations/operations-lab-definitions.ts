...
const dhcp = allocateDhcpLease({ pool: { network: '192.168.20.0', prefix: 24, start: '192.168.20.100', end: '192.168.20.102', excluded: ['192.168.20.100'] }, leases: [] }, 'PC-A');
...
const stp = calculateSpanningTree([{ id: 'SW-A', priority: 32768, mac: '00:00:00:00:00:0A' }, { id: 'SW-B', priority: 24576, mac: '00:00:00:00:00:0B' }, { id: 'SW-C', priority: 32768, mac: '00:00:00:00:00:0C' }], [{ id: 'AB', a: 'SW-A', b: 'SW-B', cost: 4, up: true }, { id: 'BC', a: 'SW-B', b: 'SW-C', cost: 4, up: true }, { id: 'AC', a: 'SW-A', b: 'SW-C', cost: 8, up: true }]);
...
const topologies: Record<string, string[]> = {
  ...
  'ipv6-address-desk': ['PC-A /64', 'LOCAL LINK', 'PC-B /64'],
  'ipv6-neighbor-desk': ['IPV6 HOST', 'R1', 'R2', 'REMOTE HOST'],
  'spanning-tree-desk': ['SW-A', 'SW-B / ROOT', 'SW-C'],
  'etherchannel-desk': ['SW-A', 'PO1 / LACP', 'SW-B'],
...
const dhcpVisualTopology = {
  description: 'PC-A and PC-B connect to SW-1. SW-1 connects to relay router R-1. R-1 reaches DHCP-1 on a different IPv4 network.',
  nodes: [
    { id: 'pc-a', label: 'PC-A', kind: 'pc' as const, detail: 'DHCP CLIENT / VLAN 20', role: 'Starts without an IPv4 address and asks for a lease.', compact: { x: 28, y: 12 }, regular: { x: 12, y: 20 }, wide: { x: 10, y: 20 } },
    { id: 'pc-b', label: 'PC-B', kind: 'pc' as const, detail: 'DHCP CLIENT / VLAN 20', role: 'Provides a second client for binding and exhaustion tests.', compact: { x: 72, y: 12 }, regular: { x: 12, y: 78 }, wide: { x: 10, y: 78 } },
    { id: 'sw-1', label: 'SW-1', kind: 'switch' as const, detail: 'CLIENT VLAN SWITCH', role: 'Floods DHCP broadcasts only inside the client VLAN.', compact: { x: 50, y: 38 }, regular: { x: 35, y: 50 }, wide: { x: 34, y: 50 } },
    { id: 'r-1', label: 'R-1', kind: 'router' as const, detail: 'RELAY / 192.168.20.1', role: 'Receives the client broadcast and relays DHCP toward the remote server.', compact: { x: 50, y: 64 }, regular: { x: 62, y: 50 }, wide: { x: 62, y: 50 } },
    { id: 'dhcp-1', label: 'DHCP-1', kind: 'server' as const, detail: 'SERVER / 192.168.10.5', role: 'Owns the address pools, exclusions, leases, and binding table.', compact: { x: 50, y: 89 }, regular: { x: 88, y: 50 }, wide: { x: 89, y: 50 } },
  ],
  links: [
...
const authoredVisualTopologies: Record<string, ReturnType<typeof lineTopology>> = {
  'transport-service-desk': lineTopology('transport', [['CLIENT PC', 'pc', 'APPLICATION CLIENT', 'Starts the transport exchange.'], ['R-1', 'router', 'IP FORWARDER', 'Forwards by IP information rather than application ports.'], ['APPLICATION SERVER', 'server', 'LISTENING SERVICE', 'Receives traffic for an open TCP or UDP port.']]),
  'dns-resolution-desk': lineTopology('dns', [['PC-A', 'pc', 'STUB RESOLVER', 'Asks its configured recursive resolver.'], ['RECURSIVE DNS', 'server', 'CACHE + RECURSION', 'Checks cache and follows referrals when needed.'], ['ROOT / TLD', 'server', 'REFERRAL SERVICE', 'Points the resolver toward the responsible authority.'], ['AUTHORITATIVE DNS', 'server', 'ZONE RECORDS', 'Returns the current record for its zone.']]),
  'acl-policy-desk': lineTopology('acl', [['PC-A', 'pc', 'SOURCE / 192.168.10.10', 'Creates the test traffic tuple.'], ['R-1', 'router', 'ACL ON G0/0 IN', 'Evaluates the ordered rules at the configured interface and direction.'], ['WEB-1', 'server', 'HTTPS / TCP 443', 'Receives only traffic permitted through the policy.']]),
  'nat-translation-desk': lineTopology('nat', [['PC-A', 'pc', 'INSIDE LOCAL', 'Starts with a private source address and port.'], ['R-1', 'router', 'NAT INSIDE / OUTSIDE', 'Selects eligible traffic and creates translation state.'], ['WEB-1', 'server', 'OUTSIDE SERVICE', 'Replies to the translated global address and port.']]),
  'ipv6-address-desk': lineTopology('ipv6-address', [['PC-A', 'pc', '2001:DB8:10::10/64', 'Uses one valid global unicast address.'], ['SW-1', 'switch', 'LOCAL ETHERNET LINK', 'Connects the two IPv6 interfaces on one link.'], ['PC-B', 'pc', '2001:DB8:10::20/64', 'Uses another address in the same /64.']]),
  'ipv6-neighbor-desk': lineTopology('ipv6-neighbor', [['PC-A', 'pc', 'SOURCE HOST', 'Selects a local neighbor or default router as next hop.'], ['R-1', 'router', 'FE80::1', 'Advertises router information and routes remote traffic.'], ['R-2', 'router', 'FE80::2', 'Provides the next routed hop.'], ['PC-B', 'pc', 'REMOTE HOST', 'Requires a usable return route.']]),
  'spanning-tree-desk': lineTopology('stp', [['SW-A', 'switch', 'PRIORITY 32768', 'Compares bridge information and selects a root path.'], ['SW-B', 'switch', 'PRIORITY 24576', 'Becomes root when it has the lowest bridge ID.'], ['SW-C', 'switch', 'PRIORITY 32768', 'Keeps one redundant path from forwarding ordinary frames.']], [[0, 2, 'F0/2', 'F0/2']]),
  'etherchannel-desk': lineTopology('etherchannel', [['SW-A', 'switch', 'LACP ACTIVE', 'Negotiates compatible physical members.'], ['SW-B', 'switch', 'LACP PASSIVE', 'Forms the other endpoint of Port-channel1.']], [[0, 1, 'F0/2', 'F0/2']]),
  'route-source-desk': lineTopology('route-source', [['PC-A', 'pc', 'SOURCE LAN', 'Creates traffic for the destination.'], ['R-1', 'router', 'ROUTE CANDIDATES', 'Matches prefixes, compares sources, and installs one best route.'], ['PC-B', 'pc', 'DESTINATION LAN', 'Represents the target reached by the installed route.']]),
  'ospf-area-desk': lineTopology('ospf', [['R-1', 'router', 'ROUTER ID 1.1.1.1', 'Forms a neighbor and calculates routes from the LSDB.'], ['R-2', 'router', 'ROUTER ID 2.2.2.2', 'Connects both OSPF links in area 0.'], ['R-3', 'router', 'ROUTER ID 3.3.3.3', 'Advertises the remote destination prefix.']]),
  'network-operations-capstone': lineTopology('operations-capstone', [['OFFICE PC', 'pc', 'IPV4 + IPV6 CLIENT', 'Starts the required and unauthorized test flows.'], ['ACCESS CORE', 'switch', 'VLAN / STP / LACP', 'Provides resilient Layer 2 forwarding.'], ['EDGE R-1', 'router', 'OSPF / ACL / PAT', 'Routes, filters, and translates eligible traffic.'], ['SERVICE-1', 'server', 'DHCP / DNS / APPLICATION', 'Provides the required office services.'], ['IPV6 BRANCH', 'pc', 'REMOTE IPV6 HOST', 'Tests Neighbor Discovery and static return routing.']]),
};
...
const authoredBriefings: Record<string, Omit<OperationsLabBriefing, 'taskChecklist' | 'lessonIds'>> = {
  'transport-service-desk': {
    ...
    startingState: ['Client PC is 192.168.10.10.', 'Application Server is 192.168.10.20.', 'R-1 forwards IP traffic and does not choose application ports.'],
  ...
  'dns-resolution-desk': {
    ...
    startingState: ['PC-A has no cached answer.', 'Recursive DNS is reachable at 192.168.10.53.', 'The authoritative server owns the netbite.test zone.'],
    workedExample: { title: 'EXAMPLE / PRINTER RECORD', steps: ['PC-A asks its configured recursive resolver for printer.netbite.test.', 'The resolver finds no current cache entry.', 'Authority returns A 192.168.10.30 with TTL 30.', 'A second identical query before expiry uses the cached A record.'], result: 'The TTL permits temporary reuse; it does not make the authoritative record permanent.' },
  },
  'acl-policy-desk': {
    ...
    startingState: ['PC-A is 192.168.10.10.', 'WEB-1 is 192.168.20.20 and listens on TCP 443.', 'Traffic from PC-A enters R-1 through G0/0.'],
  ...
  'nat-translation-desk': {
    ...
    startingState: ['PC-A uses private address 192.168.10.10.', 'R-1 has an inside and an outside interface.', 'The outside server replies to the translated global tuple.'],
  ...
  'ipv6-address-desk': {
    ...
    startingState: ['PC-A and PC-B share one Ethernet link.', 'The exercise prefix is 2001:db8:10::/64.', 'No DHCPv6 or live address timer is used.'],
  ...
  'ipv6-neighbor-desk': {
    ...
    startingState: ['PC-A uses IPv6 Neighbor Discovery, not ARP.', 'R-1 advertises link-local address fe80::1.', 'The remote branch is 2001:db8:20::/64.'],
  ...
  'spanning-tree-desk': {
    ...
    startingState: ['SW-A, SW-B, and SW-C form a physical triangle.', 'All three links are active.', 'SW-B starts with the lowest bridge priority.'],
  ...
  'etherchannel-desk': {
    ...
    startingState: ['SW-A and SW-B have two candidate physical links.', 'Both links must agree on speed and switchport settings.', 'At least one LACP endpoint must use active mode.'],
    workedExample: { title: 'EXAMPLE / ONE COMPATIBLE MEMBER', steps: ['Set SW-A F0/1 to active and SW-B F0/1 to passive.', 'Match speed and trunk mode.', 'Match the allowed VLAN list.', 'Inspect whether F0/1 becomes an active Po1 member.'], result: 'LACP can form the logical channel only from compatible negotiated members.' },
  },
  'route-source-desk': {
    goal: 'Separate prefix matching, route-source preference, protocol metric, installation, and final forwarding.',
    startingState: ['R-1 has connected, static, OSPF, and default candidates.', 'The destination is 192.168.20.20.', 'Only matching prefixes enter the selection step.'],
...
export const operationsLabDefinitions: Record<string, OperationsLabDefinition> = Object.fromEntries([
  ...
  common('dhcp-lease-desk','OPERATE A DHCP POOL','DORA, LEASES, EXCLUSIONS, AND RELAY',['UDP endpoints','IPv4 subnet ranges'],'DHCP BINDINGS',[
    ...
    ['renew','Renew the existing lease','What should PC-A do while its current binding is still known?','RENEW PC-A BINDING',['DHCPREQUEST 192.168.20.101','DHCPACK 192.168.20.101'],why('PC-A already has a valid binding.','A known client can request its current address and refresh the modeled lease.','Renewal keeps the same address assigned to PC-A.','Inspect the binding rather than allocating a second address.'),'Renewal does not require a different address.'],
    ...
    ['release','Return one address to the pool','What happens when PC-B releases its current binding?','RELEASE PC-B BINDING',['DHCPRELEASE 192.168.20.102','ADDRESS AVAILABLE'],why('PC-B no longer needs its current address.','Removing the binding returns that address to the configured pool.','A later client can be offered 192.168.20.102.','Inspect the binding and available-address list.'),'A release does not expand the configured subnet.'],
  ...
  common('network-operations-capstone','OPERATIONS CAPSTONE','PART 1 IPV4 SMALL OFFICE / PART 2 IPV6 BRANCH',['All Network Operations modules'],'DEPENDENCY VERIFICATION',[
    ...
    ['office-services','Provide host configuration and names','What must be operational before clients can locate the internal service?','DHCP LEASE + DNS A RECORD',['PC-A 192.168.10.101/24','DNS server.netbite.test = 192.168.20.20'],why('The client needs usable IPv4 settings and a name mapping.','DHCP supplies configuration; DNS resolves the application name independently.','The client can identify the server endpoint.','Verify routed policy and reachability.'),'Address assignment and name resolution are separate dependencies.'],
