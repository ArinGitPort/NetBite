...
function genericVisualTopology(id: string, labels: string[]) {
  const nodes = labels.map((label, index): OperationsTopologyNode => {
    ...
    return { id: `${id}-node-${index}`, label, kind: inferredKind(label), detail: 'VIEW DEVICE STATUS', role: label, compact: { x: 50, y: 12 + index * (76 / Math.max(1, labels.length - 1)) }, regular: { x: portion, y: 50 }, wide: { x: portion, y: 50 } };
  });
...
function lineTopology(id: string, authored: AuthoredNode[], extraLinks: [number, number, string, string][] = []) {
  ...
  const port = (node: OperationsTopologyNode, side: 'left' | 'right') => node.kind === 'router' ? side === 'left' ? 'G0/0' : 'G0/1' : node.kind === 'switch' ? side === 'left' ? 'F0/1' : 'F0/2' : 'E0';
  const links: OperationsTopologyLink[] = nodes.slice(1).map((node, index) => ({ id: `${id}-link-${index}`, a: nodes[index].id, b: node.id, aPort: port(nodes[index], 'right'), bPort: port(node, 'left') }));
...
const integratedOperationsTopology: ReturnType<typeof lineTopology> = {
  description: 'The IPv4 Small Office connects PC1 through two resilient switches and R1 to DHCP1, DNS1, and WEB1. The IPv6 Branch connects PC2 through R2 and the routed core.',
  nodes: [
    { id: 'integrated-pc1', label: 'PC1', kind: 'pc', detail: 'IPV4 OFFICE CLIENT', role: 'Requests addressing, resolves WEB1, and generates permitted and blocked test flows.', compact: { x: 50, y: 6 }, regular: { x: 8, y: 34 }, wide: { x: 8, y: 34 } },
    { id: 'integrated-sw1', label: 'SW1', kind: 'switch', detail: 'ACCESS / LACP', role: 'Carries the user VLAN and contributes physical members to Port-channel1.', compact: { x: 50, y: 17 }, regular: { x: 24, y: 34 }, wide: { x: 24, y: 34 } },
    { id: 'integrated-sw2', label: 'SW2', kind: 'switch', detail: 'ROOT / TRUNK', role: 'Provides the second LACP endpoint and the intended spanning-tree root.', compact: { x: 50, y: 28 }, regular: { x: 40, y: 34 }, wide: { x: 40, y: 34 } },
    { id: 'integrated-r1', label: 'R1', kind: 'router', detail: 'OSPF / ACL / PAT', role: 'Routes office traffic, applies policy, and translates eligible outside flows.', compact: { x: 50, y: 39 }, regular: { x: 57, y: 34 }, wide: { x: 57, y: 34 } },
    { id: 'integrated-dhcp1', label: 'DHCP1', kind: 'server', detail: 'ADDRESS SERVICE', role: 'Owns the office DHCP pool, exclusions, and bindings.', compact: { x: 50, y: 50 }, regular: { x: 79, y: 15 }, wide: { x: 80, y: 15 } },
    { id: 'integrated-dns1', label: 'DNS1', kind: 'server', detail: 'NAME SERVICE', role: 'Stores the office service A record and answers the DNS lookup.', compact: { x: 50, y: 61 }, regular: { x: 88, y: 34 }, wide: { x: 89, y: 34 } },
    { id: 'integrated-web1', label: 'WEB1', kind: 'server', detail: 'HTTPS SERVICE', role: 'Receives the required HTTPS flow when every dependency is ready.', compact: { x: 50, y: 72 }, regular: { x: 79, y: 53 }, wide: { x: 80, y: 53 } },
    { id: 'integrated-r2', label: 'R2', kind: 'router', detail: 'IPV6 BRANCH ROUTER', role: 'Provides Router Discovery, Neighbor Discovery, and static IPv6 routing.', compact: { x: 50, y: 83 }, regular: { x: 57, y: 78 }, wide: { x: 57, y: 78 } },
    { id: 'integrated-pc2', label: 'PC2', kind: 'pc', detail: 'IPV6 BRANCH HOST', role: 'Verifies IPv6 neighbor state and forward and return reachability.', compact: { x: 50, y: 94 }, regular: { x: 26, y: 78 }, wide: { x: 26, y: 78 } },
  ],
  links: [
    { id: 'integrated-user-access', a: 'integrated-pc1', b: 'integrated-sw1', aPort: 'E0', bPort: 'F0/1' },
    { id: 'integrated-lacp-1', a: 'integrated-sw1', b: 'integrated-sw2', aPort: 'F0/23', bPort: 'F0/23' },
    { id: 'integrated-lacp-2', a: 'integrated-sw1', b: 'integrated-sw2', aPort: 'F0/24', bPort: 'F0/24' },
    { id: 'integrated-router-trunk', a: 'integrated-sw2', b: 'integrated-r1', aPort: 'G0/1', bPort: 'G0/0' },
    { id: 'integrated-dhcp', a: 'integrated-r1', b: 'integrated-dhcp1', aPort: 'G0/1', bPort: 'E0' },
    { id: 'integrated-dns', a: 'integrated-r1', b: 'integrated-dns1', aPort: 'G0/2', bPort: 'E0' },
    { id: 'integrated-web', a: 'integrated-r1', b: 'integrated-web1', aPort: 'G0/3', bPort: 'E0' },
    { id: 'integrated-ipv6-core', a: 'integrated-r1', b: 'integrated-r2', aPort: 'G0/4', bPort: 'G0/0' },
    { id: 'integrated-branch', a: 'integrated-r2', b: 'integrated-pc2', aPort: 'G0/1', bPort: 'E0' },
  ],
};
...
const authoredVisualTopologies: Record<string, ReturnType<typeof lineTopology>> = {
  ...
  'network-operations-capstone': integratedOperationsTopology,
};
...
export const operationsLabDefinitions: Record<string, OperationsLabDefinition> = Object.fromEntries([
  common('transport-service-desk', 'BUILD TRANSPORT EXCHANGES', 'TCP AND UDP ENDPOINT STATE', ['IPv4 endpoints', 'Application services'], 'CONNECTION STATE', [
    ...
    ['recovery','Recover missing TCP data','A data segment is missing. What should TCP do?','RETRANSMIT UNACKNOWLEDGED DATA',tcp.events,tcp.explanation,'TCP acknowledgments reveal which ordered bytes still need delivery.'],
  ...
  ],'This practice does not send real DNS traffic and does not cover DNSSEC, full recursive-server operation, or real-time waiting.'),
  common('acl-policy-desk','APPLY AN IPV4 TRAFFIC POLICY','ORDERED FIRST-MATCH FILTERING',['IPv4 prefixes','TCP/UDP ports'],'ACL MATCH TRACE',[
    ...
    ['match','Evaluate first match','Which rule decides this HTTPS flow?','ALLOW-WEB PERMIT',[`MATCH ${acl.matchedRuleId}`,`ACTION ${acl.action.toUpperCase()}`],why(acl.reason,'ACL entries are evaluated in order and processing stops at the first match.','The allow-web rule permits this exact flow.','Test a different source or service to expose the implicit deny.'),'Stop at the first matching rule; do not combine later rules.'],
  ...
  common('nat-translation-desk','BUILD NAT AND PAT STATE','INSIDE, OUTSIDE, AND RETURN MATCHING',['IPv4 routes','ACL traffic selection'],'TRANSLATION TABLE',[
    ...
    ['return','Match the reply','What state sends the reply to the original inside host?','LOOK UP THE TRANSLATION ENTRY',['203.0.113.10:40000','-> 192.168.10.10:49152'],why('A reply targets the allocated global tuple.','The translation table maps that tuple back to the inside local endpoint.','The return flow can be reversed correctly.','Verify routing and table lifetime assumptions.'),'The outside host does not address the private endpoint directly.'],
  ...
  common('network-operations-capstone','INTEGRATED NETWORK OPERATIONS LAB','PHASE 1 IPV4 SMALL OFFICE / PHASE 2 IPV6 BRANCH',['All Network Operations modules'],'DEPENDENCY VERIFICATION',[
    ...
    ['office-verify','Verify the IPv4 office','What proves the required exchange works?','TEST FORWARD AND RETURN PATHS',['IPV4 FORWARD PASS','IPV4 RETURN PASS','DNS SERVICE REACHED'],why('Every configured dependency is now active.','A successful service exchange requires both directions, not just one installed route.','The IPv4 office phase meets its objective.','Begin the IPv6 branch.'),'A one-way trace is incomplete evidence.'],
    ...
    ['branch-fault','Recover the injected IPv6 fault','The route is correct but its parent interface is shut. What must change?','ENABLE THE PARENT INTERFACE',['PARENT INTERFACE UP','NDP RE-RUN','IPV6 FORWARD PASS','IPV6 RETURN PASS'],why('The selected next hop depends on a disabled physical interface.','Logical IPv6 forwarding cannot use a down outgoing link.','Restoring the parent allows neighbor and route processing to complete.','Review both integrated-lab phases.'),'Changing the destination route cannot repair a disabled local interface.'],
