...
export function OperationsGuidedLab({ definition, onComplete }: { definition: OperationsLabDefinition; onComplete?: () => void }) {
  ...
  return <Screen header={<PageHeader leading={{ accessibilityLabel: 'Back from guided simulator', icon: 'arrow-left', label: definition.id === 'network-operations-capstone' ? 'BACK / COURSES' : 'BACK / MODULE', onPress: () => definition.id === 'network-operations-capstone' ? router.dismissTo(AppRoutes.courses) : returnToOwningChapter('lab', definition.id) }} status="LOCAL AUTOSAVE / V4" />}>
    <View onLayout={onLayout}>
      ...
      {!finished ? <>
        <View style={styles.objective}><Text variant="label" style={styles.panelTitle}>CURRENT OBJECTIVE / {objectiveState.replace('-', ' ').toUpperCase()}</Text><Text variant="sectionHeading" style={styles.objectiveTitle}>{authored.objective}</Text><Text variant="body" style={styles.line}>{authored.prompt}</Text><Text variant="bodySmall" style={styles.objectiveEvidence}>CURRENT EVIDENCE / {currentOutcome.message}</Text><Text variant="bodySmall" style={styles.objectiveHelp}>NEXT RECOMMENDED ACTION / {configured ? `Run ${current.actionLabel.toUpperCase()} and inspect the resulting state.` : 'Configure the required values below, then save them.'}</Text><Text variant="bodySmall" style={styles.objectiveHelp}>NEED THE METHOD? OPEN “LEARN THE SETUP” ABOVE OR REVEAL A HINT BELOW.</Text></View>
        <View style={styles.inspector}><Text variant="label" style={styles.inspectorTitle}>{workspaceTitle}</Text><Text variant="bodySmall" style={styles.line}>Change the modeled device state, save it, then trigger the current protocol action. Valid mistakes remain visible until you repair them.</Text>
