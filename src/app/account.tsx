...
export default function AccountScreen() {
  const { status, user, profile, hasPro, syncStatus, error, signOut, deleteAccount, syncNow } = useAuth();
  ...
  return <Screen>
    ...
    <View style={styles.panel}><Text variant="sectionHeading">NETBITE PRO</Text><Text variant="bodySmall">{hasPro ? 'ACTIVE / Chapters 5–12 and Network Sandbox unlocked.' : 'NOT ACTIVE / View the academic test checkout.'}</Text><AppButton label={hasPro ? 'View Pro access' : 'View NetBite Pro'} onPress={() => router.push(AppRoutes.pro)} /></View>
    ...
    <FeedbackModal visible={deleteVisible} tone="warning" eyebrow="PERMANENT ACTION" title="Delete this account?" message="Cloud progress, profile data, and NetBite entitlement records will be removed. Stripe test records may remain in the provider dashboard." detail="Your separate guest snapshot on this device is not deleted." icon="reset" onRequestClose={() => setDeleteVisible(false)} secondaryAction={{ label: 'Keep account', variant: 'secondary', onPress: () => setDeleteVisible(false) }} primaryAction={{ label: 'Delete account', onPress: () => void deleteAccount().then((nextError) => { if (nextError) { setDeleteError(nextError); setDeleteVisible(false); } else router.replace(AppRoutes.authWelcome); }) }} />
  </Screen>;
}
