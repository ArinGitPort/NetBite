    ...
    <>
      ...
      <div className="grid grid-cols-1 items-start gap-5 xl:grid-cols-[minmax(320px,.7fr)_minmax(0,1.3fr)]">
        ...
        <section className="rounded-panel border border-line bg-surface p-5 shadow-panel">
          <h2 className="text-lg">Published versions</h2>
          {rows.map((row, index) => (
            <article
              className="grid grid-cols-[54px_minmax(0,1fr)_auto] items-center gap-4 border-t border-line py-4 max-sm:grid-cols-1"
              key={row.id}
            >
              <div className="grid size-12 place-items-center rounded-control border border-signal-green font-mono text-xs text-[#9bc8bd]">
