...
import { normalizeDeviceReference } from '@/shared/device-display-names';
...
const definitions: GuidedSimulationDefinition[] = [
  ...
  { labId: 'dhcp-lease-desk', releaseState: 'released', cliEnabled: false, stages: [
    stage('pool', 'Save DHCP pool', [
      ...
      { ...number('dhcp.leaseSteps', 'Practice lease duration', 4, 'positive', 'Use four learner-controlled steps for this exercise.'), placeholder: 'EXAMPLE / 8', helpText: 'NetBite uses learner-controlled steps instead of real clock time. Use the duration supplied above.' },
      ...
      'DHCP1 may lease only 192.168.20.100 through 192.168.20.102 for this task.',
    ...
    stage('dora', 'Request client lease', [text('dhcp.client', 'Client identifier', 'PC1', 'text', 'Use the fixed client PC1 so its binding can be inspected.')], ['The client starts without an IPv4 lease.', 'Run Discover, Offer, Request, and ACK for PC1.']),
    stage('renew', 'Renew client lease', [text('dhcp.renewClient', 'Client to renew', 'PC1', 'text', 'Renew the existing PC1 binding.')], ['A known client can request its current address again.', 'Renew PC1 and inspect the refreshed binding rather than allocating another address.']),
    stage('exhaust', 'Request until exhausted', [number('dhcp.requestCount', 'Additional client requests', 2, 'positive', 'Two more requests demonstrate one final binding followed by exhaustion.')], ['Only two usable pool addresses remain after the exclusion.', 'PC1 uses one; another client uses the second; the following request receives no offer.']),
    stage('release', 'Release one binding', [text('dhcp.releaseClient', 'Client to release', 'PC2', 'text', 'Release PC2 so its address returns to the pool.')], ['A released binding no longer reserves its address.', 'Release PC2 and verify that 192.168.20.102 becomes available again.']),
  ...
  { labId: 'acl-policy-desk', releaseState: 'released', cliEnabled: true, stages: [
    ...
    stage('direction', 'Apply and verify ACL', [text('acl.interface', 'Interface', 'G0/0', 'text', 'Apply the policy where the source traffic enters.'), select('acl.direction', 'Direction', 'in', [option('Inbound', 'in', 'ip access-group NETBITE-IN in'), option('Outbound', 'out', 'ip access-group NETBITE-IN out')], 'Direction is relative to the selected router interface.')], ['The source traffic enters G0/0.', 'Apply NETBITE-IN inbound on G0/0, then test permitted and denied flows.']),
  ]},
  ...
  { labId: 'ipv6-address-desk', releaseState: 'released', cliEnabled: false, stages: [
    ...
    stage('prefix', 'Save interface addresses', [text('ipv6.hostA', 'PC1 address', '2001:db8:10::10', 'ipv6', 'PC1 must use the shared prefix.'), text('ipv6.hostB', 'PC2 address', '2001:db8:10::20', 'ipv6', 'PC2 must use the shared prefix.'), number('ipv6.prefix', 'Prefix', 64, 'prefix6', 'Use the standard /64 exercise boundary.')], ['Both hosts must share the first 64 prefix bits.', 'Configure 2001:db8:10::10/64 and 2001:db8:10::20/64.']),
  ]},
  ...
  { labId: 'spanning-tree-desk', releaseState: 'released', cliEnabled: false, stages: [
    stage('root', 'Calculate root bridge', [number('stp.priorityA', 'SW1 priority', 32768, 'positive', 'Keep SW1 at the default exercise priority.'), number('stp.priorityB', 'SW2 priority', 24576, 'positive', 'Give SW2 the lowest bridge priority.'), number('stp.priorityC', 'SW3 priority', 32768, 'positive', 'Keep SW3 at the default exercise priority.')], ['Bridge priority is compared before the MAC-derived identifier.', 'Set SW2 to 24576 while SW1 and SW3 remain 32768.']),
    stage('ports', 'Calculate port roles', [number('stp.costAB', 'SW1–SW2 cost', 4, 'positive', 'Use cost 4 for the direct link.'), number('stp.costBC', 'SW2–SW3 cost', 4, 'positive', 'Use cost 4 for the direct link.'), number('stp.costAC', 'SW1–SW3 cost', 8, 'positive', 'Use cost 8 for the redundant link.')], ['Each non-root switch selects its lowest-cost path to the root.', 'Calculate using costs 4, 4, and 8 before reading every role.']),
    ...
    stage('change', 'Fail link and recalculate', [select('stp.failedLink', 'Failed active link', 'AB', [option('SW1—SW2', 'AB'), option('No failure', 'none')], 'Disable an active link to test the redundant topology.')], ['STP recalculates from the current graph.', 'Fail AB and verify that a surviving redundant link can enter the forwarding tree.']),
  ]},
  { labId: 'etherchannel-desk', releaseState: 'released', cliEnabled: true, stages: [
    stage('mode', 'Save LACP modes', [select('lacp.modeA', 'SW1 mode', 'active', [option('Active', 'active', 'channel-group 1 mode active'), option('Passive', 'passive', 'channel-group 1 mode passive')], 'At least one endpoint must actively initiate.'), select('lacp.modeB', 'SW2 mode', 'passive', [option('Passive', 'passive', 'channel-group 1 mode passive'), option('Active', 'active', 'channel-group 1 mode active')], 'Active/passive is the guided pairing.')], ['Passive/passive does not initiate LACP in this model.', 'Configure SW1 active and SW2 passive.']),
    stage('members', 'Save member compatibility', [number('lacp.speedA', 'SW1 member speed', 1000, 'positive', 'Use matching 1 Gbit/s members.'), number('lacp.speedB', 'SW2 member speed', 1000, 'positive', 'Both endpoints need the same member speed.'), select('lacp.switchportMode', 'Switchport mode', 'trunk', [option('Trunk', 'trunk', 'switchport mode trunk'), option('Access', 'access', 'switchport mode access')], 'The inter-switch bundle must be a trunk.')], ['Bundle members must describe the same logical service.', 'Match speed and trunk mode on both endpoints.']),
  ...
  { labId: 'network-operations-capstone', releaseState: 'released', cliEnabled: true, stages: [
    stage('office-l2', 'Verify resilient Layer 2', [text('cap.vlans', 'Trunk VLAN list', '10,20', 'csv-vlan', 'The office trunk must carry VLAN 10 and 20.'), select('cap.lacp', 'LACP endpoint modes', 'active-passive', [option('Active / passive', 'active-passive', 'channel-group 1 mode active'), option('Passive / passive', 'passive-passive')], 'At least one LACP endpoint must initiate.'), text('cap.stp', 'Expected root bridge', 'SW2', 'text', 'The configured bridge priority makes SW2 the root.')], ['Build VLAN context before testing services.', 'Allow VLAN 10 and 20, form active/passive LACP, and verify SW2 as root.']),
...
const boundedCliCommands: Record<string, { stageId: string; command: string; updates: Record<string, SimulationValue> }[]> = {
  ...
  'network-operations-capstone': [
    { stageId: 'office-l2', command: 'office l2 vlan 10,20 lacp active-passive root sw-b', updates: { 'cap.vlans': '10,20', 'cap.lacp': 'active-passive', 'cap.stp': 'SW2' } },
...
function dhcpObjective(stageDefinition: GuidedSimulationStage, session: OperationsSimulationSession, explanation: SimulationExplanation): ObjectiveResult {
  ...
  if (stageDefinition.id === 'pool') {
    ...
    if (stageDefinition.id === 'dora' && !state.leases.some((lease) => normalizeDeviceReference(lease.clientId) === 'pc-a' && lease.address === '192.168.20.101')) return { accepted: true, passed: false, message: 'PC1 must receive the first available address from the configured pool.', evidence: events.map((text, index) => ({ id: `dora-${index}`, text, tone: 'warning' })), tableRows: dhcpRows(state), explanation: allocation.explanation, protocolState: { dhcp: state } };
    if (stageDefinition.id === 'renew' && normalizeDeviceReference(client) !== 'pc-a') return { accepted: true, passed: false, message: 'Renew the existing PC1 binding rather than creating another client binding.', evidence: events.map((text, index) => ({ id: `renew-${index}`, text, tone: 'warning' })), tableRows: dhcpRows(state), explanation: allocation.explanation, protocolState: { dhcp: state } };
  } else if (stageDefinition.id === 'exhaust') {
    const clients = ['PC2', 'PC3'].slice(0, Number(session.configuration['dhcp.requestCount']));
    ...
    events = [`CLIENT BROADCAST / VLAN 20`, `R1 RELAY / ${session.configuration['dhcp.relay']}`, `DHCP1 PATH / ${relay.forwarded ? 'FORWARDED' : 'STOPPED'}`, relay.reason];
...
function engineEvidence(labId: string, configuration: Record<string, SimulationValue>): string[] {
  ...
  if (labId === 'dhcp-lease-desk') {
    ...
    const first = allocateDhcpLease(state, String(configuration['dhcp.client'] ?? 'PC1'));
  ...
  if (labId === 'spanning-tree-desk') {
    const tree = calculateSpanningTree([{ id: 'SW1', priority: Number(configuration['stp.priorityA'] ?? 32768), mac: '00:00:00:00:00:0A' }, { id: 'SW2', priority: Number(configuration['stp.priorityB'] ?? 32768), mac: '00:00:00:00:00:0B' }, { id: 'SW3', priority: Number(configuration['stp.priorityC'] ?? 32768), mac: '00:00:00:00:00:0C' }], [{ id: 'AB', a: 'SW1', b: 'SW2', cost: Number(configuration['stp.costAB'] ?? 4), up: configuration['stp.failedLink'] !== 'AB' }, { id: 'BC', a: 'SW2', b: 'SW3', cost: Number(configuration['stp.costBC'] ?? 4), up: true }, { id: 'AC', a: 'SW1', b: 'SW3', cost: Number(configuration['stp.costAC'] ?? 8), up: true }]);
...
export function evaluateSimulationObjective(labId: string, stageDefinition: GuidedSimulationStage, session: OperationsSimulationSession, explanation: SimulationExplanation): ObjectiveResult {
  ...
  if (labId === 'transport-service-desk') {
    ...
    return incorrect
      ...
      : { accepted: true, passed: true, message: 'The current configuration satisfies this objective.', evidence, tableRows: evidence.map(({ text }) => text), explanation };
  }
...
export function executeOperationsCliCommand(definition: GuidedSimulationDefinition, session: OperationsSimulationSession, input: string) {
  ...
  if (bounded) return { accepted: true, configuration: { ...session.configuration, ...bounded.updates }, output: `ACCEPTED / ${Object.keys(bounded.updates).length} SETTING${Object.keys(bounded.updates).length === 1 ? '' : 'S'} UPDATED` };
...
function parseOperationsCliCommand(labId: string, command: string): { updates: Record<string, SimulationValue>; output: string } | undefined {
  ...
  if (labId === 'nat-translation-desk') {
    ...
    if (command === 'show ip nat translations') return { updates: { 'nat.returnCheck': 'table' }, output: 'TRANSLATION TABLE REQUESTED / CURRENT CONFIGURATION WILL BE USED' };
  }
  if (labId === 'ipv6-neighbor-desk') {
