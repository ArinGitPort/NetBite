...
export const chapterFive = createAdvancedChapter({
  ...
  cards: [
    ['why-subnet', 'Why would an administrator divide one /24 into smaller subnets?', 'To create smaller network and broadcast boundaries for separate groups or locations.', 'Subnetting organizes one address block; it is not a second delivery protocol.'],
    ['masks-prefixes', 'Which subnet mask corresponds to /26?', '255.255.255.192.', 'The final octet is 11000000: two network bits worth 128+64=192.'],
    ['subnet-borrowed-bits', 'Starting from /24, how many final-octet bits are borrowed for /25, /26, and /27?', '/25 borrows 1, /26 borrows 2, and /27 borrows 3.', 'Borrowing extends the network portion and leaves fewer host bits.'],
    ['host-bits-address-count', 'How many total addresses are in a /26 subnet, and why?', '64 addresses because /26 leaves 6 host bits and 2^6 = 64.', 'In a conventional subnet, two endpoints are reserved as network and broadcast addresses.'],
    ['host-bits-address-count', 'How many usable host addresses are in a conventional /27?', '30 usable hosts.', '/27 leaves 5 host bits: 2^5 = 32 total addresses, minus network and broadcast.'],
    ['subnet-block-size', 'What are the block sizes for /24, /25, /26, and /27?', '256, 128, 64, and 32 addresses respectively.', 'The block size is the distance between consecutive subnet starts.'],
    ['subnet-map', 'What are the four /26 network starts inside 192.168.10.0/24?', '192.168.10.0, 192.168.10.64, 192.168.10.128, and 192.168.10.192.', 'A /26 block advances by 64 addresses each time.'],
    ['finding-boundaries', 'Which /26 subnet contains 192.168.10.70?', '192.168.10.64/26.', 'The address falls between network start 192.168.10.64 and the next start 192.168.10.128.'],
    ['network-broadcast-hosts', 'For 192.168.10.64/26, what are the network, usable range, and broadcast addresses?', 'Network 192.168.10.64; usable 192.168.10.65–192.168.10.126; broadcast 192.168.10.127.', 'The broadcast is one address before the next subnet start, 192.168.10.128.'],
    ['subnet-repeatable-method', 'What repeatable steps locate the subnet containing an IPv4 host?', 'Find host bits, calculate block size, list network starts, choose the containing block, then mark network and broadcast endpoints.', 'Using the same order prevents unexplained boundary guesses.'],
  ],
