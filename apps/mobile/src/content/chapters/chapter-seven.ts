...
export const chapterSeven = createAdvancedChapter({
  ...
  lessons: [
    {
      ...
      example: { label: 'MISSING FRAME ADDRESS', setup: 'PC1 knows local next-hop IPv4 192.168.10.20 but not its MAC.', result: 'ARP asks the LAN for the interface that owns 192.168.10.20.' },
    ...
    },
    {
      ...
      sections: [
        { heading: 'The request carries known facts', body: 'PC1 includes its own IPv4 and MAC, operation Request, and target IPv4 192.168.10.20. The ARP target-hardware field is unknown and has no useful target value yet; it is separate from the outer Ethernet broadcast destination.' },
        { heading: 'Flood only inside this VLAN', body: 'The switch learns PC1 from the frame source and floods the broadcast through every other active port in VLAN 1. All local interfaces may inspect the question, but only the owner of 192.168.10.20 normally answers.' },
      ],
      example: { label: 'WHO HAS 192.168.10.20?', setup: 'PC1 is 192.168.10.10 with MAC 02:00:00:00:00:0A. Its cache has no entry for PC2.', presentation: 'guided', visual: { illustration: 'arp-request', stageIds: ['build', 'broadcast', 'flood', 'inspect'] }, steps: [
        { id: 'build', label: 'BUILD THE ARP QUESTION', explanation: 'PC1 supplies its sender IPv4 and MAC, asks for 192.168.10.20, and marks the operation as Request.' },
        { id: 'broadcast', label: 'ADD THE ETHERNET ENVELOPE', explanation: 'Because the target MAC is unknown, PC1 uses destination FF:FF:FF:FF:FF:FF and EtherType 0x0806.' },
        ...
        { id: 'inspect', label: 'MATCH THE TARGET IPv4', explanation: 'Each receiving host inspects the question. PC2 owns 192.168.10.20, so it prepares the reply.' },
      ...
      checkpoint: { prompt: 'PC1 knows PC2’s IPv4 address but not its MAC address. Why does PC1 send its first ARP Request to FF:FF:FF:FF:FF:FF?', correctChoiceId: 'unknown', choices: [
    ...
    },
    {
      ...
      body: 'The interface owning the requested IPv4 address builds operation Reply and places its IPv4-to-MAC mapping in the ARP message. Because PC1 included its addresses in the request, PC2 can normally place the reply inside a unicast Ethernet frame addressed directly to PC1.',
      sections: [
        { heading: 'The request teaches too', body: 'The sender fields in PC1’s request assert that 192.168.10.10 is at 02:00:00:00:00:0A. PC2 can learn or refresh that mapping before it examines whether the message is a request.' },
        { heading: 'The reply supplies the missing fact', body: 'The reply states that 192.168.10.20 is at 02:00:00:00:00:0B. PC1 stores that mapping and can finally build the separate Ethernet data frame.' },
      ...
      example: { label: 'OWNER RESPONSE', setup: 'PC2 owns 192.168.10.20 and uses MAC 02:00:00:00:00:0B.', presentation: 'guided', visual: { illustration: 'arp-reply', stageIds: ['learn-requester', 'build-reply', 'unicast', 'cache'] }, steps: [
        { id: 'learn-requester', label: 'READ THE REQUESTER', explanation: 'PC2 can learn PC1’s mapping from the sender fields carried in the request.' },
        { id: 'build-reply', label: 'SUPPLY THE OWNER MAPPING', explanation: 'PC2 sets operation Reply and identifies 192.168.10.20 as 02:00:00:00:00:0B.' },
        { id: 'unicast', label: 'RETURN DIRECTLY', explanation: 'PC2 normally unicasts the Ethernet reply to PC1 MAC 02:00:00:00:00:0A.' },
        { id: 'cache', label: 'STORE AND SEND', explanation: 'PC1 caches PC2’s mapping, then uses it as the destination MAC of the later data frame.' },
    ...
    },
    {
      ...
      example: { label: 'SECOND FRAME', setup: 'PC1 already caches 192.168.10.20 → PC2 MAC.', result: 'PC1 uses the cached destination MAC and sends without a new ARP request.' },
    ...
    },
    {
      ...
      example: { label: 'LOCAL A TO B', setup: 'PC1 192.168.10.10/24 sends to PC2 192.168.10.20/24 with an empty ARP cache.', steps: [
        { id: 'local', label: 'CHOOSE THE NEXT HOP', explanation: 'Both addresses belong to 192.168.10.0/24, so PC2 itself is the local next hop.' },
        { id: 'request', label: 'BROADCAST THE QUESTION', explanation: 'PC1 asks who owns 192.168.10.20.' },
        { id: 'reply', label: 'LEARN THE OWNER', explanation: 'PC2 unicasts its MAC mapping back to PC1.' },
        { id: 'send', label: 'BUILD THE DATA FRAME', explanation: 'PC1 uses PC2’s learned MAC as the destination.' },
    ...
    },
    {
      ...
      example: { label: 'REMOTE NEXT HOP', setup: 'PC1 192.168.10.10/24 sends to 192.168.20.20 through gateway 192.168.10.1.', steps: [
        { id: 'remote', label: 'IDENTIFY REMOTE DESTINATION', explanation: 'The destination is outside PC1’s 192.168.10.0/24 subnet.' },
        { id: 'gateway', label: 'CHOOSE THE GATEWAY', explanation: 'The gateway is the next hop on PC1’s local link.', value: '192.168.10.1' },
        { id: 'resolve', label: 'ARP LOCALLY', explanation: 'PC1 resolves the gateway IPv4 address to the gateway MAC.' },
      ...
      checkpoint: { prompt: 'PC1 sends to a remote subnet. Which IPv4 address should it ARP for?', correctChoiceId: 'gateway', choices: [
        { id: 'gateway', label: 'LOCAL DEFAULT GATEWAY', feedback: 'Correct. The gateway is the local next hop for remote delivery.' },
        { id: 'remote', label: 'REMOTE HOST', feedback: 'The remote host is beyond the local broadcast domain, so PC1 cannot resolve it directly.' },
  ...
  cards: [
    ...
    ['arp-local-sequence', 'PC1 sends to PC2 in the same subnet. Whose MAC address must PC1 resolve?', 'PC2’s MAC address.', 'For local delivery, the destination host itself is the local next hop.'],
    ['arp-next-hop', 'PC1 sends to a remote subnet. Whose MAC address must PC1 resolve?', 'The default gateway’s local-interface MAC address.', 'ARP resolves only the local next hop, not the final remote host across the router.'],
