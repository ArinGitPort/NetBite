...
export default function WorkshopAssessmentScreen() {
  ...
  const choose = (questionId: string, choiceId: string) => { const next = { ...answers, [questionId]: choiceId }; setAnswers(next); if (assessment.mode === 'graded') saveDraft({ classId, versionId: entry.manifest.versionId, assessmentId, requestId, answers: next, updatedAt: new Date().toISOString() }); };
  ...
  return <Screen header={<PageHeader leading={{ accessibilityLabel: 'Back to workshop', icon: 'close', label: 'CLOSE', onPress: () => router.replace(workshopRoute(classId)) }} status={assessment.mode === 'graded' ? 'GRADED' : 'PRACTICE'} />}>
