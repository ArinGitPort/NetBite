  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 items-start gap-5 xl:grid-cols-[minmax(320px,.7fr)_minmax(0,1.3fr)]">
        <form className="grid gap-4 rounded-panel border border-line bg-surface p-6 shadow-panel" onSubmit={upload}>
          ...
          <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-control border border-copy bg-copy px-4 text-xs font-semibold text-canvas hover:bg-white hover:text-canvas disabled:pointer-events-none disabled:border-line/60 disabled:bg-raised/70 disabled:text-muted/75 [&_svg]:size-4" disabled={busy}>
            <Upload />
        ...
        <section className="rounded-panel border border-line bg-surface p-6 shadow-panel">
          ...
          <div className="grid gap-3">
            {rows.map((asset) => (
              <article className="grid grid-cols-[74px_minmax(0,1fr)_44px] items-center gap-3 rounded-control border border-line bg-canvas p-3 max-sm:grid-cols-[60px_minmax(0,1fr)]" key={asset.id}>
